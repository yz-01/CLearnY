	...
	<div class="main">
		...
		<div class="content">
					...
					<div>
						...
						?>
							<div class="contentAns">
								<input type="radio" id="Q1a" name="<?php echo $j;?>" value="<?php echo $row_answer["asnwer_id"];?>" required>
			...
			<div class="contentBlockRightButton">
				...
				<input class="contentBlockRightButtonSubmit" type="submit" name="Sendbtn" value="Submit"/>
				<div></div>
			</div>
		</div>
		</form>
...
{
	$result_question_php = mysqli_query($connect, "SELECT * FROM question WHERE quiz_id = '$id'");
	$row_question_php = mysqli_fetch_assoc($result_question_php);
	$j=0;
	do
	{
		$ans = $_POST[$j];
		mysqli_query($connect, "INSERT INTO learner_answer(quiz_id, question_id, answer_id, learner_id)VALUES('$id', '$row_question_php[question_id]', '$ans', '$row[learner_id]')");
		$j++;
	}while($row_question_php = mysqli_fetch_assoc($result_question_php));
	mysqli_query($connect, "INSERT INTO result_quiz(quiz_id, learner_id)VALUES('$id', '$row[learner_id]')");
	?>
		<script> 
					swal(
					{
