...
<body>
	...
	<?php
		...
		{
			$id = $_GET["id"];
			$result_answer = mysqli_query($connect, "SELECT * FROM answer WHERE asnwer_id = '$id'");
	...
	<div class="main">
		<div class="content">
			<div class="contentQuizTitle">
				<i class="fa fa-gamepad" aria-hidden="true" style="margin-right:10px;"></i>Edit Answer
			</div>
				...
				<form name="upload_educator" action="" method="POST" enctype='multipart/form-data'>
					<div class="contentQuizContent">
								...
								<div class="btn" style="margin-bottom:5px;">
									<input type='text' name='ans' value='<?php echo $row_answer["answer_content"];?>' style='padding:5px; margin-left:5px;'  />
									<input type="submit" name="Sendbtn" value="Save">
								</div>
								<div style="color: red; font-size:10px;">(Please update your answer after this)</div>
					</div>
				</form>
			</div>
...
if(isset($_POST["Sendbtn"]))
{
	...
	$result_question = mysqli_query($connect, "UPDATE answer SET answer_content='$ans' where asnwer_id='$row_answer[asnwer_id]'");
	?>
		<script>
			swal(
                {
					...
					{
						window.location.href = "EducatorEditQuiz.php?buy&id=<?php echo $row_answer['question_id']?>";
					});
		</script>
	<?php
}
