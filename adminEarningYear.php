...
<body>
	...
	<div class="main">
		...
		<?php
			$pur_p= mysqli_query($connect, "SELECT * from purchasing_new GROUP BY YEAR(purchasing_date)");
			$row_pur_p= mysqli_fetch_assoc($pur_p);
			do
			{
				$n = date("Y", strtotime($row_pur_p["purchasing_date"]));
				$pur_total= mysqli_query($connect, "SELECT * from purchasing_new where YEAR(purchasing_date) = '$n'");
				$row_pur_total= mysqli_fetch_assoc($pur_total);
				$t = 0;
				do
				{
					$t = $t + $row_pur_total["amount"];
				}while($row_pur_total= mysqli_fetch_assoc($pur_total));
				?>
				<div class="content">
					<div class="contentClearnY">
						<div class="contentClearnYTitle">
							C-learnY's earning (<?php echo $n;?>)
						</div>
								...
								<div class="contentEduTitleContent">
									...
									<?php
										...
										if($count_course != 0)
										{
											do
											{
												$pur= mysqli_query($connect, "SELECT * from purchasing_new where course_id='$row_course[course_id]' and YEAR(purchasing_date) = '$n'");
												$row_pur= mysqli_fetch_assoc($pur);
												$count_pur= mysqli_num_rows($pur);
												if($count_pur != 0)
												{
													$add = $row_pur["amount"];
												}
			...
			<?php
			}while($row_pur_p= mysqli_fetch_assoc($pur_p));
			?>
