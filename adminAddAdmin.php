					...
					<div class="contentDetailsArrange">
						...
						<div>
							<select name = "stat" required>
								...
								<option value="n">NormalAdmin</option>
							</select>
						</div>
...
if(isset($_POST["submit"])) 	
{
	...
	$result_email = mysqli_query($connect,"SELECT * FROM admin where admin_email = '$lemail'");
	...
	if($conp == $lpassword)
	{
		...
		{
			mysqli_query($connect,"INSERT INTO admin(admin_name,admin_password,admin_phone_number,admin_email,state,role)
			VALUES('$lname','$lpassword','$lphone','$lemail', 'ACTIVE','$sta')");
			?>
			<script>
				swal(
					{
