			...
			<div class="contentDetails">
				<form name="edudetail" method="POST">
					<div class="contentDetailsArrange">
						...
						<div>
							<input type="text" name="name" size="40" value="<?php echo $row["educator_name"];?>" required />
						</div>
						...
						<div>
							<input type="email" name="email" size="40" value="<?php echo $row["educator_email"];?>" required disabled />
						</div>
						...
						<div>
							<input type="password" name="password" size="40" value="<?php echo $row["educator_password"];?>" disabled required />
						</div>
						...
						<div>
							<input type="tel" name="phone" size="40" value="<?php echo $row["educator_phone_number"];?>"required />
						</div>
...
if(isset($_POST["submit"])) 	
{
	...
	$result_phone = mysqli_query($connect,"SELECT * FROM educator where educator_phone_number = '$lphone' AND educator_id != '$row[educator_id]'");
	$count_phone = mysqli_num_rows($result_phone);
		if($count_phone != 0 || strlen($row["educator_phone_number"]) < 10)
		{
			if(strlen($row["educator_phone_number"]) < 10)
			{
			?>
				<script>
					swal(
		...
		else if($row['educator_phone_number'] == $lphone)
		{
			mysqli_query($connect,"UPDATE educator SET educator_name='$lname', educator_phone_number='$lphone', state='$sta' where educator_id='$row[educator_id]'");
			?>
			<script>
				swal(
					{
					...
					{
						window.location.href = "adminEduProfile.php";
					});
			</script>
			<?php
		}
