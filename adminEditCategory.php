...
<body>
	...
	<div class="sidebar">
		...
		{
			$id = $_GET["id"];
			$result_cate = mysqli_query($connect, "SELECT * FROM category WHERE category_id = '$id'");
			...
			<div class="contentDetails">
				<form name="edudetail" method="POST">
					<div class="contentDetailsArrange">
						...
						<div>
							<input type="text" name="name" size="40" value="<?php echo $row_cate["category_name"];?>" required/>
						</div>
					</div>
...
if(isset($_POST["sub"]))
{
	...
	mysqli_query($connect,"UPDATE category SET category_name='$name' where category_id='$row_cate[category_id]'");
	?>
	<script>
					swal(
					{
