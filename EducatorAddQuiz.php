			...
			<div class="contentForm">
				<form>
					<div class="contentQuizDetails">
						<div>How many question do you have?</div>	
						<input type='number' name='numOfQuiz' min='1' max='10' style='padding:5px;'required />
						<input type='hidden' name='numofcourse' style='width:1%;'value='<?php echo $row_course["course_id"];?>'/>
						<input type="submit" name="submit" value="Upload">
				</form>
				<form name="upload_educator" action="" method="POST" enctype='multipart/form-data'>
						<div>Practice Quiz Name</div>
						<input type='text' name='quizname' style='padding:5px;' placeholder='Travel around the world'required />
						<div>Expected time period (minutes)</div>
						<input type='number' name='expectedTIme' min='0' style='padding:5px;' placeholder='15'required />
					</div>
					<?php
						if(isset($_GET["submit"]))
						{
							...
							$array = array("A", "B", "C", "D");
							$num = $_GET["numOfQuiz"];
							$coursenmae = $_GET["numofcourse"];
							for($i = 0; $i < $num; $i++)
							{
							?>
								<div class="contentQuizContent">
									<hr style="height:1px; width:100%" >
									<div>Question <?php echo $i+1;?></div>
									<textarea name='<?php echo $i+1;?>' rows='4' cols='50' maxlength='500' style='padding:5px; margin-bottom:10px;' required></textarea>
									<?php
										for($j=0; $j<4; $j++)
										{
										?>
									...
									<select name = '<?php echo $t;?>' style='font-size:15px; padding:5px;'required>
...
if(isset($_POST["Sendbtn"]))
{
	...
	$quiz = mysqli_query($connect, "INSERT INTO quiz(quiz_time, course_id, quiz_title, question_num)VALUES('$expectedTIme', '$coursenmae', '$quizname', $num)");
	$row_quiz= mysqli_fetch_assoc(mysqli_query($connect, "SELECT * FROM quiz where quiz_title = '$quizname'"));
	$k=1000;
	$t=100;
	$array = array("a", "b", "c", "d");
	for($i = 0; $i < $row_quiz['question_num']; $i++)
	{
		$questions = $_POST[$i+1];
		$answer = $_POST[$t];
