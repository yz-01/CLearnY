...
{
	$email = $_POST["email"];
	$result_email = mysqli_query($connect,"SELECT * FROM learner where learner_id = '$row_learner[learner_id]'");
			...
			swal(
                {
					...
					{
						window.location.href = "Learner_reset_password.php?buy&id=<?php echo $row_learner['learner_id']?>";
					});
		</script>
		<?php
	}
