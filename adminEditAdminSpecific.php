...
?>
<html>
<title>Admin edit Educator profile</title>
			...
			<div class="contentDetails">
				<form name="edudetail" method="POST">
					<div class="contentDetailsArrange">
						<div>
							<i class="fa fa-user" aria-hidden="true"></i>Admin Name
						</div>
						<div>
							<input type="text" name="name" size="40" value="<?php echo $row["admin_name"];?>" required />
						</div>
						...
						<div>
							<input type="email" name="email" size="40" value="<?php echo $row["admin_email"];?>" required disabled />
						</div>
						...
						<div>
							<input type="password" name="password" size="40" value="<?php echo $row["admin_password"];?>"  required />
						</div>
						...
						<div>
							<input type="tel" name="phone" size="40" value="<?php echo $row["admin_phone_number"];?>"required />
						</div>
						...
						<div>
							<input type="password" name="conp" size="40" value="<?php echo $row["admin_password"];?>"  required />
						</div>
						...
						<div >
							<i class="fa fa-check" aria-hidden="true"></i>Role
						</div>
						<div>
...
if(isset($_POST["submit"])) 	
{
	...
	$result_phone = mysqli_query($connect,"SELECT * FROM admin where admin_phone_number = '$lphone'");
	$count_phone = mysqli_num_rows($result_phone);
	if($conp == $lpassword)
	{
		if($count_phone != 0 || strlen($lphone) < 10)
		{
			if(strlen($lphone) < 10)
			{
			?>
			...
			else if($row['admin_phone_number'] == $lphone)
			{
				mysqli_query($connect,"UPDATE admin SET admin_name='$lname', admin_phone_number='$lphone', state='$sta', admin_password='$conp', role='$role' where admin_id='$row[admin_id]'");
				?>
				<script>
					swal(
						{
						...
						{
							window.location.href = "adminAdminProfile.php";
						});
				</script>
				<?php
			}
